...
//     When DEV_MODE = true the app uses your current GPS position as the
//     office centre so the flow always shows "Inside Approved Zone" for testing.
const DEV_MODE = true;
...
export default function CheckInLocationScreen({ onConfirm, onValidationFailed, onBack }: Props) {
  ...
  const fetchLocation = async () => {
    ...
    try {
      ...
      // so the distance is always 0 and the flow shows "Inside Approved Zone".
