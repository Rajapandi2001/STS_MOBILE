...
interface ManagerLeaveManagementScreenProps {
  onBack?: () => void;
...
export default function ManagerLeaveManagementScreen({
  onBack,
  onNavigate,
  initialParams,
}: ManagerLeaveManagementScreenProps) {
  ...
  // Handle incoming new leave submitted from Apply Screen
  useEffect(() => {
    if (initialParams?.newLeave) {
      setMyLogs(prev => {
        // Prevent duplicate append
        if (prev.some(log => log.id === initialParams.newLeave.id)) {
          return prev;
        }
        return [initialParams.newLeave, ...prev];
      });
    }
  }, [initialParams]);
        ...
        >
          ...
          <View style={styles.balanceButtonsRow}>
            <TouchableOpacity 
              ...
              onPress={() => onNavigate?.('manager_create_leave')}
            >
              <Text style={styles.balanceBtnPrimaryText}>Apply Leave</Text>
            </TouchableOpacity>
            <TouchableOpacity 
              activeOpacity={0.8}
  ...
  return (
    <View style={[styles.mainContainer, { backgroundColor: colors.bgScreen }]}>
      ...
      >
        ...
        >
          {activeTab === 'my_logs' ? renderMyLogs() : renderTeamLogs()}
        </ScrollView>
      </KeyboardAvoidingView>
